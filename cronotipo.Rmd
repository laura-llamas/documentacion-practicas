...
Eliminar muestras duplicadas, columnas+filas con valores NA, columnas con varianza 0 (eliminar ruido) y la variable "cronotipo_numero_cronotipo_number", ya que el cronotipo se asigna directamente a partir de ella
...
df_final <- df_final %>% select(-c(cronotipo_numero_cronotipo_number))

```

Asegurar que la variable "crono_tipo_etiqueta_cronotipo_label" es de tipo factor
...
Eliminar variables que no aportan información relevante para facilitar el manejo de los datos.

```{r}

df_final <- df_final %>% select(-c(visita_paciente.x, form.x, visita_paciente.y, form.y, visita_paciente, form))
...
df_final$crono_tipo_etiqueta_cronotipo_label es la columna a partir de la cual se realiza la partición
...
rf_particion_2 <- createDataPartition(df_final$crono_tipo_etiqueta_cronotipo_label, p=0.85, list = FALSE)
rf_train_data <- df_final[rf_particion_2,]
rf_test_data <- df_final[-rf_particion_2,]

x_train_prueba <- rf_train_data[, colnames(rf_train_data) != "crono_tipo_etiqueta_cronotipo_label"]
y_train_prueba <- rf_train_data[, colnames(rf_train_data) == "crono_tipo_etiqueta_cronotipo_label"]
...
set.seed(1234)
seeds <- list(
  sample.int(1000, 200), #Para el fold 1 
  sample.int(1000, 200), #Para el fold 2
  sample.int(1000, 200), #Para el fold 3
  sample.int(1000, 200), #Para el fold 4
  sample.int(1000, 200), #Para el fold 5 
  sample.int(1000, 1)    #Una sola semilla final
)
...
saveRDS(resultado_rfe_rf, file = "resultado_rfe_rf")
...
df_post_rfe <- df_final[,c(resultado_rfe_rf$optVariables, "crono_tipo_etiqueta_cronotipo_label")]
rf_train_data <- rf_train_data[,c(resultado_rfe_rf$optVariables, "crono_tipo_etiqueta_cronotipo_label")]
rf_test_data <- rf_test_data[,c(resultado_rfe_rf$optVariables, "crono_tipo_etiqueta_cronotipo_label")]
...
crono_tipo_etiqueta_cronotipo_label ~ . indica que se emplea como variable dependiente (crono_tipo_etiqueta_cronotipo_label) y el resto como predictores (.)
...
rf_training.svm.lineal <- train(crono_tipo_etiqueta_cronotipo_label ~ ., data = rf_train_data,
                      method = "svmLinear2",
...
saveRDS(rf_training.svm.lineal, "modelo_rf_svml_krono")
...
rf_training.svm.radial <- train(crono_tipo_etiqueta_cronotipo_label ~ ., data = rf_train_data,
                      method = "svmRadial",
                      tuneLength = 15,
                      metric = "Accuracy",
                      preProcess = c("center", "scale"),
...
saveRDS(rf_training.svm.radial, "modelo_rf_svmr_krono")
...
rf_training.svm.polinomial <- train(crono_tipo_etiqueta_cronotipo_label ~ ., data = rf_train_data,
                      method = "svmPoly",
...
saveRDS(rf_training.svm.polinomial, "modelo_rf_svmp_krono")
...
rf_training.rf.1 <- train(crono_tipo_etiqueta_cronotipo_label ~ ., data = rf_train_data,
                      method = "ranger",
...
saveRDS(rf_training.rf.1, "modelo_rf_rf_krono")
...
rf_training.nb.1 <- train(crono_tipo_etiqueta_cronotipo_label ~ ., data = rf_train_data, 
                 method = "naive_bayes",
                 trControl = control.cv.10,
                 preProcess = c("center", "scale"),
...
saveRDS(rf_training.nb.1, "modelo_rf_nb_krono")
...
rf_matriz.confusion.rf <- confusionMatrix(rf_664_predict_rf, rf_test_data$crono_tipo_etiqueta_cronotipo_label)
rf_matriz.confusion.rf

rf_matriz.confusion.rf$byClass[c(row.names(rf_matriz.confusion.rf$byClass)),c("Sensitivity","Specificity")]
rf_matriz.confusion.rf$table

```

Definición de un función que añada una fila que muestre la media de las métricas de la matriz de confusión

```{r}

add_mean_metrics <- function(confusion_matrix) {
  
  metrics <- confusion_matrix$byClass[, c("Sensitivity", "Specificity")]
  
  mean_metrics <- colMeans(metrics, na.rm = TRUE)
  
  mean_metrics_row <- matrix(mean_metrics, nrow = 1)
  
  row.names(mean_metrics_row) <- "Mean"
  metrics_with_mean <- rbind(metrics, mean_metrics_row)
  return(metrics_with_mean)
}

```
 
Añadir fila con medias a la matriz de confusión de Random Forest

```{r}

rf_matriz.confusion.rf.mean <- add_mean_metrics(rf_matriz.confusion.rf)
rf_matriz.confusion.rf.mean
...
rf_matriz.confusion.svml <- confusionMatrix(rf_664_predict_svml, rf_test_data$crono_tipo_etiqueta_cronotipo_label)
rf_matriz.confusion.svml

rf_matriz.confusion.svml$byClass[c(row.names(rf_matriz.confusion.svml$byClass)),c("Sensitivity","Specificity")]
rf_matriz.confusion.svml$table

rf_matriz.confusion.svml.mean <- add_mean_metrics(rf_matriz.confusion.svml)
rf_matriz.confusion.svml.mean
...
rf_matriz.confusion.svmr <- confusionMatrix(rf_664_predict_svmr, rf_test_data$crono_tipo_etiqueta_cronotipo_label)
rf_matriz.confusion.svmr

rf_matriz.confusion.svmr$byClass[c(row.names(rf_matriz.confusion.svmr$byClass)),c("Sensitivity","Specificity","Precision","F1")]
rf_matriz.confusion.svmr$table

rf_matriz.confusion.svmr.mean <- add_mean_metrics(rf_matriz.confusion.svmr)
rf_matriz.confusion.svmr.mean
...
rf_matriz.confusion.svmp <- confusionMatrix(rf_664_predict_svmp, rf_test_data$crono_tipo_etiqueta_cronotipo_label)
rf_matriz.confusion.svmp

rf_matriz.confusion.svmp$byClass[c(row.names(rf_matriz.confusion.svmp$byClass)),c("Sensitivity","Specificity","Precision","F1")]
rf_matriz.confusion.svmp$table

rf_matriz.confusion.svmp.mean <- add_mean_metrics(rf_matriz.confusion.svmr)
rf_matriz.confusion.svmp.mean
...
rf_matriz.confusion.nb <- confusionMatrix(rf_664_predict_nb, rf_test_data$crono_tipo_etiqueta_cronotipo_label)
rf_matriz.confusion.nb 

rf_matriz.confusion.nb$byClass[c(row.names(rf_matriz.confusion.nb$byClass)),c("Sensitivity","Specificity","Precision","F1")]
rf_matriz.confusion.nb$table

rf_matriz.confusion.nb.mean <- add_mean_metrics(rf_matriz.confusion.svmr)
rf_matriz.confusion.nb.mean

``` 

Crear un data frame en el que las filas correspondan a los distintos algoritmos (RF, SVMP, SVML, SVMR, NB) y las columnas incluyan la media de cada métrica (Sensibilidad, Especificidad, Precisión Global).
Para ello, emplearemos una versión modificada de la función add_mean_metrics que definimos antes

```{r}

df_mean_metrics <- function(confusion_matrix) {
  ...
  all_metrics <- c(mean_metrics, accuracy)
  
  return(all_metrics)
}
```

Almacenar en variables las medias de las métricas 
Unimos las variables usando rbind
Convertir a data.frame para una mejor manipulación y posterior visualización

```{r}

rf_metrics <- df_mean_metrics(rf_matriz.confusion.rf)
svmp_metrics <- df_mean_metrics(rf_matriz.confusion.svmp)
svml_metrics <- df_mean_metrics(rf_matriz.confusion.svml)
svmr_metrics <- df_mean_metrics(rf_matriz.confusion.svmr)
nb_metrics <- df_mean_metrics(rf_matriz.confusion.nb)
