...
df_final <- df_final %>% mutate(imc = peso_kg/(altura_cm/100)^2 ) %>% 
  mutate(imc_categoria = cut(imc, c(0,18.5,25,30,Inf), labels = c("bajo peso","normopeso","sobrepeso","obeso") , right = F) ) 
...
df_final <- df_final %>% select(-c(peso_kg, altura_cm, imc, visita_paciente.x, form.x, visita_paciente.y, form.y, visita_paciente, form))
...
saveRDS(resultado_rfe_rf, file = "resultado_rfe_rf_imc")
...
saveRDS(rf_training.svm.lineal, "modelo_rf_svml_imc")
...
saveRDS(rf_training.svm.radial, "modelo_rf_svmr_imc")
...
saveRDS(rf_training.svm.polinomial, "modelo_rf_svmp_imc")
...
saveRDS(rf_training.rf.1, "modelo_rf_rf_imc")
...
saveRDS(rf_training.nb.1, "modelo_rf_nb_imc")
...
Definición de un función que añada una fila con la media de las métricas de la matriz de confusión
...
imc4_metrics_df <- as.data.frame(metrics_df)
imc4_metrics_df
...
saveRDS(imc4_metrics_df, "imc4_metrics_df")
